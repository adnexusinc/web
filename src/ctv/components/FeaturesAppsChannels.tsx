...
import data from '../data/features-apps-channels.json';
import { Link } from 'react-router-dom';

export default function FeaturesAppsChannels() {
  return (
        ...
        <section className="pt-32 pb-20 bg-gradient-to-b from-black via-gray-900 to-black">
          <div className="container mx-auto px-6">
            <div className="max-w-4xl mx-auto text-center">
              <h1 className="text-6xl md:text-7xl font-bold mb-6 bg-gradient-to-r from-white via-indigo-100 to-indigo-200 bg-clip-text text-transparent">
                {data.hero.headline}
              </h1>
          ...
          <div className="container mx-auto px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {data.features && data.features.map((feature: any, index: number) => (
                <div key={index} className="bg-gray-900 border border-gray-800 rounded-2xl p-8 hover:border-gray-700 transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-900/20">
                  <div className="mb-4"><Sparkles className="h-10 w-10 text-indigo-400" /></div>
            ...
            {data.success_stories && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {data.success_stories.slice(0, 9).map((story: any, index: number) => (
                  <div key={index} className="bg-gray-900 border border-gray-800 rounded-xl p-6 hover:border-indigo-600 transition-all duration-300">
                    <div className="text-5xl font-bold text-indigo-400 mb-2">{story.metric}</div>
