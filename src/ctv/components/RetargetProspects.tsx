...
import data from '../data/retarget-prospects.json';
import { Link } from 'react-router-dom';

export default function RetargetProspects() {
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="pt-24">
        <section className="pt-32 pb-20 bg-gradient-to-b from-black via-gray-900 to-black">
          <div className="container mx-auto px-6">
            <div className="max-w-4xl mx-auto text-center">
              <h1 className="text-6xl md:text-7xl font-bold mb-6 bg-gradient-to-r from-white via-pink-100 to-pink-200 bg-clip-text text-transparent">
                {data.hero.headline}
              </h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
                  {Object.values(data.stats).map((stat: any, index) => (
                    <div key={index} className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-2xl p-6">
                      <div className="text-4xl font-bold text-pink-400 mb-2">{stat.value}</div>
                      <div className="text-sm text-gray-400">{stat.label}</div>
                      {stat.change && <div className="text-green-400 text-sm mt-1">{stat.change}</div>}
                    </div>
                  ))}
                </div>
              )}
        ...
        <section className="py-20 bg-black">
          <div className="container mx-auto px-6">
            <div className="text-center mb-16">
              ...
              <p className="text-gray-400 text-lg max-w-3xl mx-auto">{data.customer_count}</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {data.features.map((feature: any, index: number) => (
                <div key={index} className="bg-gray-900 border border-gray-800 rounded-2xl p-8 hover:border-gray-700 transition-all duration-300 hover:shadow-2xl hover:shadow-pink-900/20">
                  <div className="mb-4"><DollarSign className="h-10 w-10 text-pink-400" /></div>
          ...
          <div className="container mx-auto px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {data.success_stories.slice(0, 9).map((story: any, index: number) => (
                <div key={index} className="bg-gray-900 border border-gray-800 rounded-xl p-6 hover:border-pink-600 transition-all duration-300">
                  <div className="text-5xl font-bold text-pink-400 mb-2">{story.metric}</div>
          ...
          <div className="container mx-auto px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 max-w-6xl mx-auto">
              {data.related_goals.map((goal: any, index: number) => (
                <Link key={index} to={goal.url} className="bg-gray-800 border border-gray-700 rounded-xl p-6 text-center hover:border-pink-500 hover:bg-gray-700 transition-all duration-300">
        ...
        <section className="py-20 bg-gradient-to-b from-black to-gray-900">
          <div className="container mx-auto px-6">
            <div className="max-w-4xl mx-auto text-center">
              <h2 className="text-4xl md:text-5xl font-bold mb-6">Ready to Retarget Your Audience?</h2>
              <p className="text-xl text-gray-400 mb-12">Start reaching 120M households across 500+ premium CTV channels today</p>
